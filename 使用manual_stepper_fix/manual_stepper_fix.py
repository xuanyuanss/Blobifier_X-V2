...
class ManualStepperFix:
    def __init__(self, config):
        ...
        gcode.register_mux_command('MANUAL_STEPPER_FIX', "STEPPER",
                                   stepper_name, self.cmd_MANUAL_STEPPER_FIX,
                                   desc=self.cmd_MANUAL_STEPPER_FIX_help)
    ...
    # 增加相对移动方法
    def _submit_move_relative(self, movetime, movepos, speed, accel):
        cp = self.rail.get_commanded_position()
        dist = movepos
        axis_r, accel_t, cruise_t, cruise_v = force_move.calc_move_time(
            dist, speed, accel)
    ...
    # 增加相对移动方法
    def do_move_relative(self, movepos, speed, accel, sync=True):
        self.sync_print_time()
        self.next_cmd_time = self._submit_move_relative(self.next_cmd_time, movepos,
                                                        speed, accel)
        self.rail.generate_steps(self.next_cmd_time)
        self.trapq_finalize_moves(self.trapq, self.next_cmd_time + 99999.9,
                                  self.next_cmd_time + 99999.9)
    ...
    cmd_MANUAL_STEPPER_FIX_help = "Command a manually configured stepper"
    def cmd_MANUAL_STEPPER_FIX(self, gcmd):
        ...
        if homing_move:
            ...
            # 增加相对移动判断条件
            relative = gcmd.get_int('RELATIVE', 0)
            if relative:
                self.do_move_relative(movepos, speed, accel, sync)
            else:
...
def load_config_prefix(config):
    return ManualStepperFix(config)
